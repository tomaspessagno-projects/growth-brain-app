...
// medido = de Mixpanel/HubSpot (azul). supuesto = editable (ámbar). Resultado en azul oscuro.
...
// "De dónde sale el número", en criollo: la cuenta es ingreso POR MES = socios nuevos × cuota ($90.000).
function Narrative({ inp, a }: { inp: MarginInputs; a: ScenarioAssumptions }) {
  const socios = convertedSocios(inp, a);
  const monthly = monthlyRevenueFromScenario(inp, a);
  const lifetime = marginFromScenario(inp, a); // valor de vida total (dato secundario)
  const monthlyMargin = monthly * a.marginPct; // margen (no facturación) por mes
  const p: React.CSSProperties = { fontSize: 13.5, lineHeight: 1.9, color: '#3a4a5c', margin: 0 };
  const foot: React.CSSProperties = { fontSize: 11.5, color: '#8696a7', marginTop: 10, lineHeight: 1.55 };

  const footnote = (
    <div style={foot}>
      Es <b>facturación</b> nueva por mes (recurrente: la siguen pagando mientras están activos). El <b>margen</b> es ~{pct(a.marginPct)} de eso ≈ {fmtArsShort(monthlyMargin)}/mes.
      Si mirás a largo plazo, cada socio deja ≈ {fmtArs(lifetime / Math.max(1, socios))} de valor de vida en ~{a.retentionMonths} meses (total {fmtArsShort(lifetime)}).
    </div>
  );
    ...
    return (
      <>
        <p style={p}>
          ...
          Si la mejora recupera el <Sup>{pct(a.recovery)}</Sup> de ellas{(inp.recMult ?? 1) !== 1 ? <> (y como esta variante pega menos, ×{inp.recMult})</> : null} <i>(tu supuesto — editalo abajo)</i>,{' '}
          son <Res>{fmtN(recovered)} personas</Res> que completan los datos. De esas, asumimos que <Sup>{pct(a.datoCapita)}</Sup> se vuelve socio <i>(supuesto)</i> → <Res>{fmtN(socios)} socios nuevos</Res> por mes.{' '}
          Cada socio paga la cuota de <Med>{fmtArs(ARPU_MENSUAL_ARS)}/mes</Med>.{' '}
          <br />
          <span style={{ fontSize: 16 }}>Ingreso nuevo por mes: <Res>{fmtN(socios)} socios × {fmtArs(ARPU_MENSUAL_ARS)} = {fmtArsShort(monthly)} / mes</Res>.</span>
        </p>
        {footnote}
      </>
    );
    ...
    return (
      <>
        <p style={p}>
          ...
          La tasa de cierre está <Med>{pct(inp.gap ?? 0)}</Med> por debajo de la meta: cerrar esa brecha son <Res>{fmtN(socios)} socios</Res> más, que pagan <Med>{fmtArs(ARPU_MENSUAL_ARS)}/mes</Med>.{' '}
          <br />
          <span style={{ fontSize: 16 }}>Ingreso/mes que se destraba: <Res>{fmtN(socios)} × {fmtArs(ARPU_MENSUAL_ARS)} = {fmtArsShort(monthly)} / mes</Res> (de un stock, se destraba una vez).</span>
        </p>
        {footnote}
      </>
    );
  ...
  return (
    <>
      <p style={p}>
        ...
        Si se cierran al <Med>{pct(inp.winRate ?? 0)}</Med> esperado, son <Res>{fmtN(socios)} socios</Res>, que pagan <Med>{fmtArs(ARPU_MENSUAL_ARS)}/mes</Med>.{' '}
        <br />
        <span style={{ fontSize: 16 }}>Ingreso/mes que se destraba: <Res>{fmtN(socios)} × {fmtArs(ARPU_MENSUAL_ARS)} = {fmtArsShort(monthly)} / mes</Res> (de un stock).</span>
      </p>
      {footnote}
    </>
  );
...
// Input NUMÉRICO de entrada libre: escribís el valor a tu antojo (sin tope de slider). Mantiene un
...
export default function ScenarioPanel({
  ...
  useEffect(() => {
    ...
    return () => { alive = false; };
  }, [recId, baseAssumptions, editable.length]);

  const monthly = useMemo(() => monthlyRevenueFromScenario(inputs, a), [inputs, a]);
  const isBase = eq(a, baseAssumptions);
  const delta = baseMargin > 0 ? monthly / baseMargin - 1 : 0;
  const per = cadence === 'acumulado' ? ' / mes (del stock)' : ' / mes';
  ...
  return (
    <section style={{ marginTop: 18 }}>
      <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase', color: '#8696a7', marginBottom: 8 }}>
        ¿De dónde sale este número?{editable.length ? ' (y ajustalo a lo que vos asumís)' : ''}
      </div>

      {/* La explicación en criollo, paso a paso, terminando en el ingreso por mes */}
      <div className="glass-panel" style={{ padding: '16px 18px' }}>
        <Narrative inp={inputs} a={a} />
        <div style={{ fontSize: 11, color: '#8696a7', marginTop: 12 }}>
          <b style={{ color: '#1689C4' }}>Azul</b> = dato medido (Mixpanel/HubSpot) · <b style={{ color: '#9a6a00' }}>Ámbar</b> = supuesto{editable.length ? ' que podés editar abajo' : ''}.
        </div>
      </div>

      {editable.length > 0 && (
        <>
          {/* Comparativo motor → tu escenario */}
          <div className="glass-panel" style={{ marginTop: 10, padding: '14px 16px', display: 'flex', gap: 22, flexWrap: 'wrap', alignItems: 'flex-end' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 30, fontWeight: 800, color: isBase ? '#5b6b7f' : '#002D5F', fontFamily: 'Satoshi, sans-serif' }}>{fmtArsShort(monthly)}{per}</div>
            </div>
            {!isBase && (
          ...
          <div style={{ fontSize: 11.5, color: '#8696a7', marginTop: 10, lineHeight: 1.5 }}>
            El “recuperable” del motor sale de benchmarks CRO de e-commerce (direccional, suele quedar alto para una prepaga). Acá dejás asentado <b>lo que VOS asumís</b>. Se guarda por oportunidad (por ahora, en este navegador).
          </div>
        </>
      )}
    </section>

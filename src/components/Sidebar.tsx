...
export default function Sidebar() {
  ...
  const navItems = [
    { name: 'Resumen', path: '/', icon: <DashboardIcon /> },
    { name: 'Funnels', path: '/funnels', icon: <FunnelIcon /> },
    { name: 'Cotizador', path: '/funnel/cotizador', icon: <CotizadorIcon /> },
    { name: 'Recomendaciones', path: '/recomendaciones', icon: <RecsIcon /> },
  ...
  return (
    <aside className={`${styles.sidebar} ${isCollapsed ? styles.collapsed : ''}`}>
      ...
      <nav className={styles.nav}>
        {navItems.map((item) => {
          const isActive = pathname === item.path || (item.path !== '/' && (pathname?.startsWith(item.path + '/') ?? false));
